...
module.exports = class SessionPersistence {
  ...
  createQuestion(surveyId, questionText, questionType, options) {
    let survey = this._findSurvey(surveyId);
    if (!survey) return false;
    ...
    survey.questions.push({
      ...
      options: options.map(option => {
        return { id: nextId(), option_value: option };
      }),
    });
  ...
  updateQuestion(surveyId, questionId, questionText, type, options) {
    ...
    question.options = options.map(option => ({ id: nextId(), option_value: option }));
    return true;
  }
