...
module.exports = class PgPersistence {
  ...
  async unpublishSurvey(surveyId) {
    ...
    let resultUnpublish = dbQuery(UNPUBLISH_SURVEY, surveyId, this.username);
    let resultDelete = dbQuery(DELETE_ANSWERS, surveyId);
    let resultBoth = await Promise.all([resultUnpublish, resultDelete]);

    return resultBoth.every(query => query.rowCount > 0);
  }
