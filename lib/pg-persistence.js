...
module.exports = class PgPersistence {
  ...
  async allSurveys() {
    const ALL_SURVEYS = "SELECT * FROM surveys WHERE username = $1 ORDER BY id ASC";
    const FIND_QUESTIONS = "SELECT * FROM questions WHERE survey_id = $1 AND username = $2";
    const FIND_PARTICIPANTS = "SELECT * FROM participants WHERE survey_id = $1";

    let result = await dbQuery(ALL_SURVEYS, this.username);
    let surveys = result.rows;

    for (let index = 0; index < surveys.length; ++index) {
      let survey = surveys[index];
      let questions = await dbQuery(FIND_QUESTIONS, survey.id, this.username);
