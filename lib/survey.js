...
class Survey {
  ...
  static makeSurvey(rawSurvey) {
    let survey = Object.assign(new Survey(rawSurvey.title), {
      id: rawSurvey.id,
      results: rawSurvey.results,
    });

    rawSurvey.participants.forEach(participant => {
      survey.participants.push(Participant.makeParticipant(participant));
    });

    rawSurvey.questions.forEach(question => {
      survey.questions.push(Question.makeQuestion(question));
    });

    rawSurvey.answers.forEach(answer => {
      survey.answers.push(Answer.makeAnswer(answer));
    });
